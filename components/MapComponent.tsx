...
const createCustomIcon = (room: Room, isSelected: boolean) => {
  ...
  const html = `
    <div style="transition: all 250ms cubic-bezier(0.4, 0, 0.2, 1); transform: ${isSelected ? 'scale(1.15)' : 'scale(1)'};">
      <div style="
        background: ${isSelected ? '#FF385C' : 'white'};
        border-radius: 16px;
        padding: 8px 16px;
        box-shadow: 0 4px 16px rgba(88,204,2,0.2);
        border: 2.5px solid ${isSelected ? '#FF385C' : '#E5E5E5'};
        font-family: var(--font-sans, 'Nunito', system-ui, sans-serif);
        cursor: pointer;
      ">
        <div style="font-weight: 900; font-size: 13px; color: ${isSelected ? 'white' : '#2B2B2B'}; white-space: nowrap; letter-spacing: -0.02em;">
          ${formattedPrice}
        </div>
        <div style="font-weight: 700; font-size: 10px; color: ${isSelected ? 'rgba(255,255,255,0.8)' : '#777777'}; white-space: nowrap; margin-top: 2px;">
          ${room.property_type}
        </div>
      </div>
    </div>
