    ...
    <div className="min-h-full flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-background h-full w-full">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-surface py-8 px-4 sm:px-10 rounded-2xl border border-border-color">
          {error && (
            <div className="mb-5 text-[13px] text-error bg-error/5 border border-error/10 rounded-xl px-4 py-2.5">
              {error}
            </div>
          )}
          ...
          <button
            ...
            disabled={isLoading}
            className="flex w-full items-center justify-center gap-3 h-[48px] rounded-xl border border-border-color text-[14px] font-medium text-primary-text hover:bg-gray-50 transition-colors disabled:opacity-50"
          >
            {isLoading ? (
